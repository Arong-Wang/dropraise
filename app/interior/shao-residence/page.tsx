...
export default function ShaoResidencePage() {
  const p = getProject("shao-residence");
    ...
    <div className="pt-20">
      ...
      <section className="blog-px pt-10 pb-4">
        <div className="flex items-center gap-4 mb-6">
          ...
          <p className={`label ${blueMuted}`}>平面配置</p>
        </div>
        <div className="overflow-hidden bg-[#f8f6f5] w-full lg:w-[60%]">
          <Image src={p.floorPlan.src} alt="平面圖" width={0} height={0} sizes="80vw" className="w-full h-auto" />
          {p.floorPlan.caption && (
      ...
      <section className="blog-px pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-8 items-end">
          <div className="overflow-hidden bg-[#f8f6f5]">
            <Image src="/works/shao-residence/0.jpg" alt="邵宅室內裝修案" width={0} height={0} sizes="(max-width: 1024px) 100vw, 60vw" className="w-full h-auto" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {p.photos.map((photo, i) => {
            const beforeMap: Record<string, string> = {
              "/works/shao-residence/1.png":  "/works/shao-residence/before/1.jpg",
              "/works/shao-residence/4.png":  "/works/shao-residence/before/4.JPG",
              "/works/shao-residence/5.png":  "/works/shao-residence/before/5.JPG",
              "/works/shao-residence/11.jpg": "/works/shao-residence/before/11.JPG",
            };
            const beforeSrc = beforeMap[photo];
            return beforeSrc ? (
              <BeforeAfterSlider key={photo} beforeSrc={beforeSrc} afterSrc={photo} />
            ) : (
