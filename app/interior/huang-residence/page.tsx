...
export default function HuangResidencePage() {
  const p = getProject("huang-residence");
    ...
    <div className="pt-20">
      ...
      <section className="px-6 md:px-10">
        <div className="relative w-full aspect-[16/9] overflow-hidden">
          <Image src={p.cover} alt={`${p.title} 封面`} fill className="object-cover" priority />
        </div>
      </section>
      ...
      <section className="px-6 md:px-10 xl:px-16 2xl:px-24 pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-8 items-end">
          <div className="relative overflow-hidden bg-[#f8f6f5]">
            <Image src={p.floorPlan.src} alt="平面圖" width={2400} height={1697} className="w-full h-auto" />
            {p.floorPlan.caption && (
              <div className="px-4 py-2 border-t border-[#0000ff]/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {p.photos.map((photo, i) => (
            <div key={photo} className="relative aspect-[4/3] overflow-hidden">
              <Image src={photo} alt={`${p.title} 完工照 ${i + 1}`} fill className="object-cover hover:scale-[1.02] transition-transform duration-500" />
            </div>
          ))}
        </div>
      </section>
